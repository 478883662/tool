...
import com.zhangb.family.doctor.basedata.entity.*;
...
import com.zhangb.family.doctor.operate.dao.DoctorReimbDao;
...
public class ReimbServiceImpl implements IReimbService {
    ...
    @Autowired
    private IReimbAddBaseDataService reimbDrugService;
    @Autowired
    ...
    @Override
    public ReimbIllnessBo getNextIllness(ReimbUserInfoPO user) throws Exception {
        ...
        ReimbDealRecordPO niJiaHuRecord = recordService.getLastRecord(user.getSelfNo(), RemoteConstants.YL_LOCATION_NO);
        //查询今年来最近的一次报销记录
        ReimbDealRecordPO record = recordService.getLastRecord(user.getSelfNo());

        Date outDate = null;
        //1、今年来在泥家湖卫生室没有报销过
        if (niJiaHuRecord == null) {
            //随机获取一种要报销的病例.及出院时间
            ReimbIllnessPO reimbIllnessPO = illnessService.getRandomIllness(user,null);
            reimbIllnessBo.setIllnessName(reimbIllnessPO.getIllnessName());
            reimbIllnessBo.setIllnessNo(reimbIllnessPO.getIllnessNo());
            //第一次报销病例必须要在上半年
            ...
            //从本地库里获取病例的用药列表
            reimbIllnessBo.setReimbDrugBoList(illnessDrugService.getIllnessDrugList(reimbIllnessPO.getIllnessNo()));
            ...
            if (illnessCount >= ReimbConstants.DEFUALT_ILLNESS_COUNT) {
                ...
                //随机生成一种新病
                ReimbIllnessPO reimbIllnessPO = illnessService.getRandomIllness(user,illnessNo);
                reimbIllnessBo.setIllnessName(reimbIllnessPO.getIllnessName());
                reimbIllnessBo.setIllnessNo(reimbIllnessPO.getIllnessNo());
    ...
    public DoctorReimbResultEnum reimb(ReimbIllnessBo reimbIllnessBo) throws Exception {
        ...
        //从本地数据库里获取病例信息
        ReimbIllnessPO reimbIllnessPO = illnessService.getIllness(reimbIllnessBo.getIllnessNo());
        if (reimbIllnessPO == null) {
        ...
        //计算本次的入院时间、出院时间
        Date inDate = DateUtil.offsetDay(reimbIllnessBo.getOutDate(), NumberUtil.parseInt(reimbIllnessPO.getRestDay()));
        Date outDate = DateUtil.offsetDay(inDate, NumberUtil.parseInt(reimbIllnessPO.getHospitalDay()));
    ...
    @Override
    public void savePrintInfo(ReimbPrintInfoPO reimbPrintInfoPO) throws Exception {
        ReimbPrintInfoPO where  = new ReimbPrintInfoPO();
        where.setBizId(reimbPrintInfoPO.getBizId());
        if (BaseDao.count(where)>0){
            BaseDao.updateForValue(reimbPrintInfoPO,where);
        }else{
            BaseDao.insert(reimbPrintInfoPO);
        }
    }
    ...
    public List<DoctorReimbResultBo> reimbForYlCardAndName(String ylCard, String name) {
        ...
        try{
            ...
            for (ReimbUserInfoPO user : userList) {
     ...
     * @throws Exception
     */
    private DoctorReimbResultBo reimbForUser(ReimbUserInfoPO user) throws Exception {
        DoctorReimbResultBo doctorReimbResultBo = new DoctorReimbResultBo(user.getYlCard(),user.getName());

        ReimbDealResultPO reimbDealResultPO = new ReimbDealResultPO();
        ...
        try{
            ...
            reimbPrintInfoPO.setCreatedDate(new Date());
            reimbPrintInfoPO.setPrintState(DoctorPrintStateEnum.UN_PRINT.getCode());
            savePrintInfo(reimbPrintInfoPO);
        ...
        return doctorReimbResultBo.setCodeMsg(reimbDealResultPO.getDealCode(), reimbDealResultPO.getDealResult());
    }
